...
def plot(options):
    ...
    if not os.path.exists(os.path.join(rootdir,'Frames')):
        os.makedirs(os.path.join(rootdir,'Frames'))
    ...
    cmd = 'ffmpeg -y -i Frames/{prefix}-%04d-disp{displacementScaling:.2f}.png -f mp4 -vcodec h264 -pix_fmt yuv420p  {prefix}-disp{displacementScaling:.2f}.mp4'.format(prefix=prefix,**options.__dict__)
