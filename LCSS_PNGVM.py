...
def plot(options):
    ...
    DefineTensorExpression("InelasticStrain", "{ {e11-Temperature,e12}, {e12,e22-Temperature} }")

    #DefineScalarExpression("sigma11", "({0} + 2.*{1}) * InelasticStrain[0][0] + {0} * InelasticStrain[1][1]".format(_lambda,_mu))
    #DefineScalarExpression("sigma22", "{0} * InelasticStrain[0][0] + ({0} + 2.*{1}) * InelasticStrain[1][1]".format(_lambda,_mu))
    #DefineScalarExpression("sigma12", " 2.*{1} * InelasticStrain[1][2]".format(_lambda,_mu))
    DefineScalarExpression("sigma11", "({0} + 2.*{1}) * (e11-Temperature) + {0} * (e22-Temperature)".format(_lambda,_mu))
    DefineScalarExpression("sigma22", "{0} * (e11-Temperature) + ({0} + 2.*{1}) * (e22-Temperature)".format(_lambda,_mu))
    DefineScalarExpression("sigma12", " 2.*{1} * e12".format(_lambda,_mu))
    #DefineTensorExpression("Sigma", "{ {sigma11,sigma12}, {sigma12,sigma22} }")

    #DefineScalarExpression("EED", "(InelasticStrain[0][0]*Sigma[0][0] + 2.* InelasticStrain[0][1]*Sigma[0][1] + InelasticStrain[0][0]*Sigma[1][1])/2.")
    #DefineScalarExpression("VonMises","sqrt((contraction(Sigma,Sigma) - (Sigma[0][0]+Sigma[1][1])^2/2))")
